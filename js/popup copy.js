...
async function removeAds(files) {
    for (let i = 0; i < files.length; i++) {
        ...
        reader.onload = async function (event) {
            ...
            const modifiedFileBlob = await callLambdaFunction(files[i]);

            if (modifiedFileBlob) {
              const url = URL.createObjectURL(modifiedFileBlob);
...
async function editFiles(files) {
  ...
  async function callLambdaFunction(file) {
    const apiGatewayUrl = 'https://8r3gwub4ne.execute-api.eu-south-2.amazonaws.com/default/removeAdsLambda';
